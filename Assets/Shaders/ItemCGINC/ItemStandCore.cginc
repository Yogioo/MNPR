...
#include "UnityStandardConfig.cginc"
...
#ifndef ITEM_STANDARD_CORE_INCLUDED
    #define ITEM_STANDARD_CORE_INCLUDED
    ...
    // float3 GetNormal(float2 uv,sampler2D normalMap,float bumpScale){
        ...
        //     float3 tangentSpaceNormal = UnpackScaleNormal(UNITY_SAMPLE_TEX2D(normalMap, uv), bumpScale);
        //     return normalize(
    ...
    half3 Albedo2(float4 texcoords,float3 worldPos,float3 i_viewDirForParallax,float2 offset,float4 blendValue,float4 screenPos)
    {
        //#region Mix All Map 
        ...
        // float4 v = _CameraDepthTexture.Sample(sampler_CameraDepthTexture,UNITY_PROJ_COORD( screenPos ));
        
        // float screenDepth = LinearEyeDepth(UNITY_SAMPLE_DEPTH(v));
        ...
        half3 r = UNITY_SAMPLE_TEX2DARRAY(_MapArray,float3(rUV,1))* _TintR;
        half3 g = UNITY_SAMPLE_TEX2DARRAY(_MapArray,float3(gUV,2))* _TintG;
        half3 b = UNITY_SAMPLE_TEX2DARRAY(_MapArray,float3(bUV,3))* _TintB;
        
        //half3 m = UNITY_SAMPLE_TEX2D(_MapM,mUV)* _TintM;
        // half3 r = UNITY_SAMPLE_TEX2D(_MapR,rUV)* _TintR;
        // half3 g = UNITY_SAMPLE_TEX2D(_MapG,gUV)* _TintG;
        // half3 b = UNITY_SAMPLE_TEX2D(_MapB,bUV)* _TintB;

        //half3 m = tex2D(_MapM,mUV) * _TintM;
        // half3 r = tex2D(_MapR,rUV)*_TintR;
        // half3 g = tex2D(_MapG,gUV)*_TintG; 
        // half3 b = tex2D(_MapB,bUV)*_TintB;
        ...
        //albedo = tex2D(_MapM,mUV + texcoords.xy);
            ...
            #elif _DETAIL_LERP
                finalAlbedo = lerp (albedo, detailAlbedo, mask);
            #endif
        #endif
        return finalAlbedo;
    }
    ...
    inline FragmentCommonData MetallicSetup (float4 i_tex,float3 worldPos,float3 i_viewDirForParallax,float2 offset,float4 blendValue,float4 screenPos)
    {
        ...
        half3 specColor;
        half3 diffColor = DiffuseAndSpecularFromMetallic (Albedo2(i_tex,worldPos,i_viewDirForParallax,offset,blendValue,screenPos), metallic, /*out*/ specColor, /*out*/ oneMinusReflectivity);
    ...
    inline FragmentCommonData FragmentSetup (inout float4 i_tex, float3 i_eyeVec, half3 i_viewDirForParallax, float4 tangentToWorld[3], float3 i_posWorld,float4 screenPos)
    {
        ...
        float3 blendMap = UNITY_SAMPLE_TEX2D(_BlendTex,blendUV);
        float bR = blendMap.r;
        float bG = blendMap.g;
        float bB = blendMap.b;
        ...
        FragmentCommonData o = MetallicSetup (i_tex,i_posWorld,i_viewDirForParallax,offset.xy,blendValue,screenPos);
        ...
        float distanceDepth =saturate( i_posWorld.y*_BlendValue);
        ...
        float3 n1 = UnpackScaleNormal(tex2D(_BlendNormalMap, blendUV), _BlendNormalBumpScale);
        n1 =  normalize(
        n1.x * tang +
        n1.y * binormal +
        n1.z * norm
        ...
        //resultNormal = float3(0,1,0);
        o.normalWorld = lerp(resultNormal,o.normalWorld,distanceDepth);
