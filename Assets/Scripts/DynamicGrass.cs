...
namespace WalkingFat
{
            ...
            for (int w = 0; w < tempGrassNumW; w++)
            {
                for (int l = 0; l < tempGrassNumL; l++)
                {
                    ...
                    tempGrassPosList[w + l * tempGrassNumW] = new Vector3(
                        ...
                        UnityEngine.Random.Range(-tempPosOffset, tempPosOffset),
                        0,
                        UnityEngine.Random.Range(-tempPosOffset, tempPosOffset)
                    );
                    ...
                    temgGrassObjList[w + l * tempGrassNumW].transform.localScale = new Vector3(grassSize, UnityEngine.Random.Range(0.7f, 1.3f) * grassHeight, grassSize);
    ...
    public class Tool_Spawn { 

        public static GameObject SpawnObj(bool v1, GameObject grassModel, GameObject gameObject, string v2, Vector3 vector31, Vector3 vector32, bool v3)
        {
            GameObject model =  GameObject.Instantiate(grassModel,gameObject.transform);
            model.name = v2;
            model.transform.position = vector31;
            model.transform.eulerAngles = vector32;
            return model;
        }
    }
